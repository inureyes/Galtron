...
def runGame():
    ...
    while runGame:
        ...
        while stats.mainMenu:
            mm.checkEvents(setting, screen, stats, sb, playBtn, twoPlayBtn, aboutBtn, quitBtn, menuBtn, sel, ship,
                           aliens, bullets, eBullets)
            mm.drawMenu(setting, screen, sb, playBtn, menuBtn, twoPlayBtn, aboutBtn, quitBtn, sel)

        while stats.mainGame:
            # Game functions
            gf.checkEvents(setting, screen, stats, sb, playBtn, quitBtn, sel, ship, aliens, bullets, eBullets,
                           timestops)  # Check for events
            if stats.gameActive:
                gf.updateAliens(setting, stats, sb, screen, ship, aliens, bullets, eBullets)  # Update aliens
                gf.updateBullets(setting, screen, stats, sb, ship, aliens, bullets, eBullets, timestops)  # Update collisions
                gf.updateTimeStop(screen, stats, ship, timestops)  # Update timestop items
                ship.update(setting, screen, timestops)  # update the ship
            gf.updateScreen(setting, screen, stats, sb, ship, aliens, bullets, eBullets, playBtn, menuBtn, quitBtn, sel,
                            timestops)  # Update the screen
