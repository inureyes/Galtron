...
def updateItems(setting, screen, stats, sb, ship, aliens, bullets, eBullets, items):
    ...
    for item in items.sprites():
        if item.rect.bottom <= 0:
            items.remove(item)
    for item in items.sprites():
        disX = int((ship.rect.centerx - ship.rect.x) + (item.rect.centerx - item.rect.x)*0.7)#전투기의 가로 픽셀의 절반+아이템의 가로 픽셀의 절반
        disY = int((ship.rect.centery - ship.rect.y) + (item.rect.centery - item.rect.y)*0.7)#전투기의 세로 픽셀의 절반+#아이템의 세로 픽셀의 절반
