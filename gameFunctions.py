...
def checkEvents(setting, screen, stats, sb, playBtn, quitBtn, sel, ship, aliens, bullets, eBullets, timestops):
    ...
    for event in pg.event.get():
        ...
        # Check for key down has been pressed
            ...
            if event.key == pg.K_UP:
                ...
                if pauseBtnState == 1:
                    checkPlayBtn(setting, screen, stats, sb, playBtn, sel, ship, aliens, bullets, eBullets, timestops)
...
def checkPlayBtn(setting, screen, stats, sb, playBtn, sel, ship, aliens, bullets, eBullets, timestops):
    ...
    if not stats.gameActive and not stats.paused:
        ...
        # Reset the aliens, bullets and the items.
        aliens.empty()
        bullets.empty()
        eBullets.empty()
        timestops.empty()
...
def shipHit(setting, stats, sb, screen, ship, aliens, bullets, eBullets, timestops):
    """Respond to ship being hit"""
    if stats.shipsLeft > 0:
        sb.prepShips()
        stats.shipsLeft -= 1
        # Empty teh list of aliens and bullets and items
        aliens.empty()
        bullets.empty()
        eBullets.empty()
        timestops.empty()
...
def updateAliens(setting, stats, sb, screen, ship, aliens, bullets, eBullets):
    ...
    aliens.update(setting, screen, stats, ship, aliens, eBullets)
...
def updateBullets(setting, screen, stats, sb, ship, aliens, bullets, eBullets, timestops):
    """update the position of the bullets"""
    # check if we are colliding
    bullets.update()
    eBullets.update()
    checkBulletAlienCol(setting, screen, stats, sb, ship, aliens, bullets)
    checkEBulletShipCol(setting, stats, sb, screen, ship, aliens, bullets, eBullets, timestops)
...
def updateTimeStop(screen, stats, ship, timestops):
    """update the position of the timestop items"""
    # check if we are colliding
    timestops.update()
    checkTimeStopItemShipCol(stats, ship, timestops)
    # if item goes off screen delete it
    for timestopitem in timestops.copy():
        screenRect = screen.get_rect()
        if timestopitem.rect.top >= screenRect.bottom:
            timestops.remove(timestopitem)
...
def checkEBulletShipCol(setting, stats, sb, screen, ship, aliens, bullets, eBullets, timestops):
    ...
    for ebullet in eBullets.sprites():
        if pg.sprite.collide_mask(ship, ebullet):
            shipHit(setting, stats, sb, screen, ship, aliens, bullets, eBullets, timestops)
            sb.prepShips()


def checkTimeStopItemShipCol(stats, ship, timestops):
    """Check for collisions using collision mask between ship and timestop itmes"""
    for timestopitem in timestops.sprites():
        if pg.sprite.collide_mask(ship, timestopitem):
            timestops.remove(timestopitem)
            timestop(stats)
